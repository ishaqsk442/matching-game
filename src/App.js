...
class App extends Component {
  ...
  onClickImg = thumbnailUrl => {
    ...
    const imgcheck = imagesList.filter(
      each => each.thumbnailUrl === thumbnailUrl,
    )

    const {imageUrl} = imgcheck[0]
  ...
  renderImagesList = () => {
    ...
    return imagesList.filter(each => each.category === categoryList)
  }

  changeItems = imgTabId => {
    // const {categoryList} = this.state

    this.setState({categoryList: imgTabId})
  }

  render() {
    const filteredList = this.renderImagesList()
    const {score, time, showScore, imgUrl, categoryList} = this.state
              ...
              <ul className="tabs-list">
                {tabsList.map(eachTab => (
                  <li key={eachTab.tabId}>
                    <button
                      className={`button ${
                        categoryList === eachTab.tabId ? 'each-tab' : ''
                      }`}
                      onClick={() => this.changeItems(eachTab.tabId)}
                      type="button"
                    >
                      {eachTab.displayText}
                    </button>
                ...
                {filteredList.map(eachImg => (
                  <li key={eachImg.id}>
                    <button
                      ...
                      type="button"
                      onClick={() => this.onClickImg(eachImg.thumbnailUrl)}
                    >
                      <img
                        className="each-img-list"
